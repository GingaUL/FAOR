...
class ATFM(nn.Module):
    def __init__(self, dim):
        ...
        self.scale_conv0 = nn.Conv2d(dim, dim, 1)  # 修改32 → dim
...
class SwinTransformerBlock(nn.Module):
    def __init__(self,
                 ...
                 condition_dim=1,
                 vit_condition_type='1conv',  # 3conv
        ...
        if use_vit_condition:  # true
            ...
            if vit_condition_type == '1conv':
                ...
                self.offset_conv = nn.Sequential(nn.Conv2d(condition_dim, c_dim, 1, 1, 0, bias=True),
                                                 ...
                                                 nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                 nn.Conv2d(c_dim, 78, 1, 1, 0, bias=True))  # 输出维度改为78匹配 embed dim=156 // 2
    ...
    def forward(self, x, x_size, condition):  
        ...
        if self.use_vit_condition:  # 使用DAAB
            ...
            # print('_condition shape = ', _condition.shape)  # 32, 3, 64, 64
            # offset = self.offset_conv(_condition).permute(0,2,3,1)  # 用condition计算偏移量offset
            # print('offset shape in STB: ', offset.shape)  # 32, 64, 64, 2
            # x_warped = flow_warp(x, offset, interp_mode='bilinear', padding_mode='border')  # 用offset对特征做warp (DAAB)  # TODO 用SFT
            # print('x shape = ', x.shape)  # 32, 156, 64, 64
            # print('x_warped shape = ', x_warped.shape)  # 32, 156, 64, 64
            # print('x shape before sft = ', x.shape)
            _condition = self.offset_conv(_condition)  # offset_conv → cond_conv
            # print('_condition shape after offset_conv = ', _condition.shape)  # 32 78 16 16  应为 32 78 64 64
            # print('x shape before sft = ', x.shape)  # 32 156 64 64
            x_warped = self.atfm((x, _condition))
            # x_warped = F.relu(x_warped, inplace=True)  # 是否要加relu？
            # print('x shape after sft = ', x.shape)
            x = x.permute(0, 2, 3, 1)
...
class RSTB(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_dcn_conv:
            ...
            # self.dcn = DCNv2Pack(dim, dim, 3, padding=1)  # DCN TODO 改用SFT
            self.atfm = ATFM(dim//2)  # add SFT
    ...
    def forward(self, x, x_size, condition):
        if self.use_dcn_conv:
            ...
            # print('_condition shape = ', _condition.shape)  # 8 78 64 64
            # print('x shape = ', _x.shape)  # 8 78 64 64
            return self.patch_embed(self.atfm((_x, self.offset_conv(condition)))) + x  # RSTB中的dcn 改为 SFT
        else:
...
class OURS(nn.Module):
    ...
    def forward(self, x, sample_coords, condition, lonlat_hr, lonlat_lr, qmap):  # 增加了用于resample的h和w
        x = self.conv_first(x)  # 提取浅层特征 3→156
        condition = self.melt(torch.cat([condition, qmap], dim=1))  # 2 → 1  TODO Melt Net
        if self.use_dcn_conv:  # 是否使用DACB
            ...
            x = self.atfm((_x , self.offset_conv(condition))) + x  # offset计算偏移，再dcn卷积（Backbone结尾的DACB），最后 + x  改用SFT
        else:
