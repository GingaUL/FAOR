...
class ATFM(nn.Module):
    ...
    def flops(self, h, w):
        flops = 0
        flops += 2 * self.dim * self.dim * h * w
        flops += 2 * self.dim * 2 * self.dim * h * w
        return flops
...
class Mlp(nn.Module):
    ...
    def flops(self):
        flops = 0
        flops += 2 * (self.in_features * self.hidden_features + self.hidden_features + self.out_features)
        return flops
...
class SwinTransformerBlock(nn.Module):
        ...
        if use_atfm:
            ...
            if atfm_type == '1conv':
                ...
                self.offset_conv = nn.Sequential(nn.Conv2d(condition_dim, c_dim, 1, 1, 0, bias=True),
                                                 ...
                                                 nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                 nn.Conv2d(c_dim, c_dim // 2, 1, 1, 0, bias=True))
        
            self.atfm = ATFM(c_dim // 2)  # embedd dim // 2
        ...
        if self.use_atfm:
            ...
            if self.window_condition:
                ...
                if self.window_condition_only:
                    ...
                    # print('shape of condition = ', condition.shape, 'shape of condition_wind = ', condition_wind.shape)  # 1 1 113 227, 1 2 112 224
    ...
    def flops(self):
        ...
        # self.offset_conv
        flops += 2 * self.condition_dim * self.c_dim * h * w
        flops += 2 * self.c_dim * self.c_dim * h * w
        flops += 2 * self.c_dim * self.c_dim // 2 * h * w

        flops += self.mlp.flops()
        flops += self.dim * h * w
        flops += self.atfm.flops(h=h, w=w)
        return flops
...
class RSTB(nn.Module):
    ...
    def flops(self):
        ...
        # conv & atfm
        flops += 2 * self.condition_dim * self.c_dim * h * w
        flops += 2 * self.c_dim * self.dim // 2 * h * w
        flops += self.atfm.flops(h=h, w=w)
